...
class AdvancedOCRExtractor:
    ...
    def extract_from_image(self, image_path, display_confidence=False, handwriting=False):
        """Extract text from an image file with preprocessing options"""
        ...
        print(f"💾 Preprocessed image saved: {base_name}_processed.png")
    ...
    def extract_from_folder(self, folder_path, output_file=None, display_confidence=False, handwriting=False):
        ...
        for image_file in sorted(image_files):
            ...
            text = self.extract_from_image(image_path, display_confidence, handwriting)
...
def main():
    print("🔤 Advanced OCR Text Extractor\n")
    ...
    # Initialize OCR
    ocr = AdvancedOCRExtractor(languages=languages)
    ...
    # Ask about text type
    text_type = input("\nIs the text handwritten? (y/n, default: n): ").strip().lower()
    handwriting = text_type == 'y'
    
    try:
        if choice == "1":
            # Single image
            image_path = input("\nEnter image file path: ").strip()
            text = ocr.extract_from_image(image_path, display_confidence=True, handwriting=handwriting)
            ...
            results = ocr.extract_from_folder(folder_path, output_file, handwriting=handwriting)
